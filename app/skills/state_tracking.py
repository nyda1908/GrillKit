...
def process_evaluation(ctx: Context, node_input: EvaluationResult) -> Event:
    """Processes the evaluation result, updates state, and resets turn-level variables."""
    evaluations = ctx.state.get("evaluations", [])
    weak_areas = ctx.state.get("weak_areas", [])
    
    # Append the latest evaluation
    ...
    # Increment the question index to move to the next main question
    current_index = ctx.state.get("current_index", 0)
    current_index += 1
    
    # Reset turn-level variables and route back to the interview flow in "main" phase
    return Event(
        state={
